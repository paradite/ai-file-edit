...
describe('File Edit Tool with OpenRouter - Multiple Files', () => {
  let fileEditTool: FileEditTool;
  const testDir = path.join(process.cwd(), 'sample-openrouter-multiple');
  ...
  test('should edit multiple files in allowed directory using OpenRouter', async () => {
    ...
    // Test editing multiple files in allowed directory using OpenRouter
    const response = await fileEditTool.processQuery(
      `Please modify all five JavaScript files to use multiplication operations instead of their current operations.
       ...
       Update the console.log statements to use the new operation.`,
      true,
    );
